...
class ClusterLoss(nn.Module):
    ...
    def forward(self, c_i, c_j):
        ...
        # print(f"sim_shape:{sim.size()}")
        sim_i_j = torch.diag(sim, self.class_num)
        # print(f"sim_i_j_shape:{sim_i_j.size()}")
        sim_j_i = torch.diag(sim, -self.class_num)
        # print(f"sim_j_i_shape:{sim_j_i.size()}")

        positive_clusters = torch.cat((sim_i_j, sim_j_i), dim=0).reshape(N, -1)
        # print(f"positive_clusetr_shape：{positive_clusters.size()}")
        negative_clusters = sim[self.mask].reshape(N, -1)
        # print(f"negative_clusters_shape：{negative_clusters.size()}")

        labels = torch.zeros(N).to(positive_clusters.device).long()
        # print(f"labels_shape:{labels.size()}")
        logits = torch.cat((positive_clusters, negative_clusters), dim=1)
        # print(f"logits_shape:{logits.size()}")
        loss = self.criterion(logits, labels)
        loss = self.temperature * loss / N
